...
import copy
from stockproject.mainapp.models import StockDetail
...
class StockConsumer(AsyncWebsocketConsumer):
    ...
    def addToCeleryBeat(self,stockpicker):
        ...
        if len(task)>0:     #task already exists
            task = task.first()
            ...
            for x in stockpicker:
                if x not in args:
                    args.append(x)
            task.args = json.dumps([args])  #dump data into celery beat arguments
            task.save()
        else:
    ...
    def selectUserStocks(self):
        user = self.scope["user"]
        user_stocks =user.stockdetail_set.value_list('stock', flat = True)
